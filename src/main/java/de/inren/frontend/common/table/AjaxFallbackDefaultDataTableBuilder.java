...
public final class AjaxFallbackDefaultDataTableBuilder<T extends Serializable> implements Serializable {
	...
	/**
	 * 
	 * Add a value to the row. This value will be displayed as is. The headline for
	 * this row will be resolved as "property.label" from the StringResources The
	 * value will be resolved with a PropertyModel of the default model with name
	 * property. There is no sort option.
	 * 
	 * @param property
	 * @return this for chaining
	 */
	 ...
	 * method. If the list is more complex, you should consider using an individual
	 * column.
	 * 
	 * @param listProperty
	 * @param itemProperty
	...
	private PropertyColumn<T, String> createBooleanPropertyColumn(final String property, boolean sortable) {
		if (sortable) {
			return new BooleanPropertyColumn<T>(new StringResourceModel(property + LABEL, component, null), property,
					property, property);
		} else {
			return new BooleanPropertyColumn<T>(new StringResourceModel(property + LABEL, component, null), property,
					property);
	...
	public AjaxFallbackDefaultDataTableBuilder<T> addDatePropertyColumn(String property, String format,
			boolean sortable) {
		columns.add(createDatePropertyColumn(property, format, sortable));
		return this;
	}

	private PropertyColumn<T, String> createDatePropertyColumn(final String property, String format, boolean sortable) {

		return new DateColumn(new StringResourceModel(property + LABEL, component, null), format, property, property,
				property);
	}
